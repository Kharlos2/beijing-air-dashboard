...
def load_models():
    if LR_PATH.exists() and TREE_PATH.exists() and SCALER_PATH.exists():
        ...
        try:
            ...
            print(f"✅ Modelos cargados desde {MODEL_DIR}")
        except Exception as e:
            print(f"❌ Error cargando modelos: {e}")
            print(f"   Buscando en: {MODEL_DIR.resolve()}")
            print(f"   Archivos encontrados: {list(MODEL_DIR.iterdir()) if MODEL_DIR.exists() else 'carpeta no existe'}")
            raise
        return lr_model, tree_model, scaler
